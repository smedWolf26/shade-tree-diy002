...
import { createSession, deleteSessionByTokenHash, findSessionByTokenHash } from '../data/sessions.repository.js'
...
auth.post('/register', async (c) => {
  ...
  if (existing) throw new ApiError(409, 'CONFLICT', 'A user with that email already exists.')
  ...
  return sendResource(c, { accessToken, refreshToken, tokenType: 'Bearer' }, 201)
})
...
auth.post('/login', async (c) => {
  ...
  const credentialsError = new ApiError(401, 'UNAUTHORIZED', 'Invalid email or password.')
  ...
  return sendResource(c, { accessToken, refreshToken, tokenType: 'Bearer' })
})
...
auth.post('/refresh', async (c) => {
  ...
  return sendResource(c, { accessToken, tokenType: 'Bearer' })
})
